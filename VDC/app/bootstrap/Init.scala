...
class Init @Inject() (lifecycle: ApplicationLifecycle, config: Configuration) {
  ...
  private var grpcDalUrl = ""
  private var grpcDalPort = 5002
  if (config.has("dal.url")) {
    grpcDalUrl = config.get[String]("dal.url")
  }
  if (config.has("dal.port")) {
    grpcDalPort = config.get[Int]("dal.port")
  }
